...
$$ \sigma_{t}^{2} = \frac{(\vec{\mu} - r_{f} \, \vec{1})^2}{ (\vec{\mu} - r_{f} \, \vec{1})^\intercal \, V^{-1} \, (\vec{\mu} - r_{f} \, \vec{1})}  = \frac{a \, r^2 - 2 \, b \, r + c}{\left| b - a \, r \right|} $$

Efficient frontier without borrowing:

$$ \sigma_{\ast}(r_{\ast}; r_{f}) = XXX $$

or equivalently

$$ r_{\ast}(\sigma_{\ast}; r_{f}) = XXX $$
