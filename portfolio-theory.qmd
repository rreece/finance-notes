...
$$ \hat{\mu}_{p} = \frac{M_{p}}{n} \,, \qquad \hat{\mu} = \frac{M_{1}}{n} \,, \qquad \hat{\sigma}^2 = \frac{M_{2}}{n} $$

Online mean:

$$ \delta \equiv x_n - \mu_{n - 1} $$

$$ \mu_{n} = \mu_{n-1} + \frac{\delta}{n} $$

Online variance:

$$ M_{2,n} = M_{2,n-1} + \delta \left( \delta - \frac{\delta}{n} \right) = M_{2,n-1} + \frac{n-1}{n} \delta^2 $$

$$ \hat{\sigma}^2 = \frac{M_{2,n}}{n-1} $$

where the $n-1$ includes Bessel's correction for sample variance.

Online covariance:

$$ C_{n} = C_{n-1} + \frac{n-1}{n} \delta_x \delta_y $$

$$ \hat{V}_{xy} = \frac{C_{n}}{n-1} $$

Matrix form:

$$ C_{n} = {C}_{n-1} + \frac{n-1}{n} \left( \vec{x}_{n} - \vec{\mu}_{n-1} \right) \left( \vec{x}_{n} - \vec{\mu}_{n-1} \right)^\intercal $$
