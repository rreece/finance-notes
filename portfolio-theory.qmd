...
-   Estimate the factor premiums, $f_{j}$, usually from historical return: $f_{j} = \mathbb{E}_{t}(F_{jt}) = \bar{F}_{j}$.
-   Expected return: $\hat{r}_{i} = \bar{r}_\mathrm{f} + \sum_{j} \hat{\beta}_{ij} \, f_{j}$, where looking forward, it is usually assumed that $\alpha_{i} = 0$.

It can be shown that

$$ \big[ \hat{\beta}^{\intercal} \big]_{ji} = \sum_k \big[ \mathrm{Cov}(f, f)^{-1} \big]_{jk} \, \big[ \mathrm{Cov}(f, r) \big]_{ki} $$

where in the simple case of CAPM, $f_\mathrm{m} = r_\mathrm{m} - r_\mathrm{f}$, and note that in general $\mathrm{Cov}(x - x_0, y) = \mathrm{Cov}(x, y)$.
