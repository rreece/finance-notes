...
$$ C_{n}(x, y) = C_{n-1} + (x_n - \bar{x}_{n - 1}) (y_n - \bar{y}_n) $$

$$ C_{n}(x, y) = C_{n-1} + \frac{n-1}{n} (x_n - \bar{x}_{n - 1}) (y_n - \bar{y}_{n - 1}) = C_{n-1} + \frac{n-1}{n} \delta_x \delta_y $$

$$ \hat{V}_{xy} = \frac{C_{n}(x, y)}{n-1} $$

Matrix form:

$$ C_{n} = {C}_{n-1} + \left( \vec{x}_{n} - \vec{\mu}_{n-1} \right) \left( \vec{x}_{n} - \vec{\mu}_{n} \right)^\intercal $$
...
$$ \hat{V}_{xy} = \frac{C_{n}}{n-1} $$

Noting that the update term for the online covariance is a term in a scatter matrix, $S$,
...
Weighted covariance: TODO: Does this work for covariance or just variance?
