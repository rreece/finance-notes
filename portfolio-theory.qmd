...
-   Luenberger, D.G. (1998). Investment Science. [^Luenberger1998a]
-   Palomar, D.P. (2025). [*Portfolio Optimization: Theory and Application*](https://portfoliooptimizationbook.com/). [^Palomar2025]
...
[^Luenberger1998a]: @Luenberger_1998_Investment_Science\.
[^Palomar2025]: @Palomar_2025_Portfolio_Optimization_Theory_and_Application\.


### Markowitz portfolio problem

Given an expected covariance, $V$, and expected returns, $\vec{\mu}$,
...
$$ r = \vec{w}^\intercal \, \vec{\mu} = \sum_i w_{i} \, \mu_{i} $$
...
-   Fama, E.F. (1970). [Efficient capital markets: A review of theory and empirical work](http://www.e-m-h.org/Fama70.pdf). [^Fama1970]
...
![Illustrative assets and an example efficient frontier.](img/markowitz-example.png){#fig-markowitz-example}
...
From which one can see that it is always the case that $\beta_\mathrm{m} = 1$ and $\alpha_\mathrm{m} = 0$.
...
-   Luenberger, D.G. (1998). *Investment Science*. [^Luenberger1998b]
...
[^Luenberger1998b]: @Luenberger_1998_Investment_Science\.
...
$$ r_{it} - r_{\mathrm{f}t} = \alpha_{i} + \beta^\mathrm{m}_{i} \, (r_{\mathrm{m}t} - r_{\mathrm{f}t}) + \beta^\mathrm{s}_{i} \, \mathrm{SMB}_{t} + \beta^\mathrm{v}_{i} \, \mathrm{HML}_{t} + \varepsilon_{it} $$

Five-factor model:

$$ r_{it} - r_{\mathrm{f}t} = \alpha_{i} + \beta^\mathrm{m}_{i} \, (r_{\mathrm{m}t} - r_{\mathrm{f}t}) + \beta^\mathrm{s}_{i} \, \mathrm{SMB}_{t} + \beta^\mathrm{v}_{i} \, \mathrm{HML}_{t} + \beta^\mathrm{p}_{i} \, \mathrm{RMW}_{t} + \beta^\mathrm{i}_{i} \, \mathrm{CMA}_{t} + \varepsilon_{it} $$

-   $r_\mathrm{m} - r_\mathrm{f}$: The return spread between the capitalization weighted stock market and cash.
...
$$ r_{it} - r_{\mathrm{f}t} = \alpha_{i} + \beta^\mathrm{m}_{i} \, (r_{\mathrm{m}t} - r_{\mathrm{f}t}) + \beta^\mathrm{s}_{i} \, \mathrm{SMB}_{t} + \beta^\mathrm{v}_{i} \, \mathrm{HML}_{t} + \beta^\mathrm{mom}_{i} \, \mathrm{PR1YR}_{t} + \varepsilon_{it} $$

On using momentum:

-   Jegadeesh, N. & Titman, S. (1993). [Returns to buying winners and selling losers: Implications for stock market efficiency](http://relative-performance.technicalanalysis.org.uk/JegadeeshTitman1993.pdf). [^Jegadeesh1993]

[^Carhart1997]: @Carhart_1997_On_persistence_in_mutual_fund_performance\.
[^Jegadeesh1993]: @Jegadeesh_1993_Returns_to_buying_winners_and_selling_losers\.


### Asness six-factor model

-   Asness, C.S. & Frazzini, A. (2013). [The Devil in HML's Details](https://doi.org/10.3905/jpm.2013.39.4.049). [^Asness2013]
...
$$ r_{it} - r_{\mathrm{f}t} = \alpha_{i} + \beta^\mathrm{m}_{i} \, (r_{\mathrm{m}t} - r_{\mathrm{f}t}) + \beta^\mathrm{s}_{i} \, \mathrm{SMB}_{t} + \beta^\mathrm{v}_{i} \, \mathrm{HML}_{t} + \beta^\mathrm{p}_{i} \, \mathrm{RMW}_{t} + \beta^\mathrm{i}_{i} \, \mathrm{CMA}_{t} + \beta^\mathrm{mom}_{i} \, \mathrm{UMD}_{t} + \varepsilon_{it} $$
...
-   Fidelity. (2016). [Putting factors to work](https://www.etf.com/docs/PuttingFactors_Work_Fidelity.pdf).
-   Fidelity. (2016). [An overview of factor investing](https://www.fidelity.com/bin-public/060_www_fidelity_com/documents/brokerage/overview-factor-investing.pdf).

$$ r_{it} - r_{\mathrm{f}t} = \alpha_{i} + \sum_{j} \beta_{ij} \, f_{jt} + \varepsilon_{it} $$

-   Regression with historical data gives $\hat{\alpha}_{i}$ and $\hat{\beta}_{ij}$.
-   $\beta_{ij}$ is called the factor loading or factor sensitivity for factor $j$.
-   Estimate the factor premiums, $\hat{f}_{j}$, usually from historical return: $\hat{f}_{j} = \mathbb{E}_{t}(f_{jt})$.
-   Expected return: $\hat{r}_{i} = \bar{r}_\mathrm{f} + \sum_{j} \hat{\beta}_{ij} \, \hat{f}_{j}$, where looking forward, it is usually assumed that $\alpha_{i} = 0$.

It can be shown from OLS that

$$ \big[ \hat{\beta}^{\intercal} \big]_{ji} = \sum_k \big[ \mathrm{cov}(\vec{f}, \vec{f})^{-1} \big]_{jk} \, \big[ \mathrm{cov}(\vec{f}, \vec{r}) \big]_{ki} $$

The $\beta$ vector over factors can be calculated for each asset independently.

$$ \hat{\vec{\beta}}_{i} = \mathrm{cov}(\vec{f}, \vec{f})^{-1} \cdot \mathrm{cov}(\vec{f}, r_{i}) $$

In the simple case of CAPM, there is a single factor, $f_\mathrm{m} = r_\mathrm{m} - r_\mathrm{f}$.
...
-   Life-cycle strategy
    -   Modigliani, F. & Brumberg, R. (1954). Utility analysis and the consumption function: An interpretation of cross-section data. [^Modigliani1954]
    -   Scott, J.S., Shoven, J.B., Slavov, S.N. & Watson, J.G. (2023). The life-cycle model implies that most young people should not save for retirement. [^Scott2023]
    -   Anarkulova, A., Cederburg, S., & O'Doherty, M.S. (2025). [Beyond the status quo: A critical assessment of lifecycle investment advice](https://ssrn.com/abstract=4590406). [^Anarkulova2025b]

[^Anarkulova2025b]: @Anarkulova_2025_Beyond_the_status_quo_A_critical_assessment\.
...
[^Modigliani1954]: @Modigliani_1954_Utility_analysis_and_the_consumption_function\.
[^Rockafellar2000]: @Rockafellar_2000_Optimization_of_conditional_value_at_risk\.
[^Scott2023]: @Scott_2023_The_life_cycle_model_implies_that_most_young\.
...
1.  Sensitivity of portfolio weights to the estimates of $\hat{\mu}$ and $\hat{V}$.
    -   Uncertainty estimation; Error propagation
...
-   Error propagation; resampling the efficieny frontier
-   Michaud, R.O. (1989). The Markowitz optimization enigma: Is 'optimized' optimal? [^Michaud1989]
-   Michaud, R.O. & Michaud, R.O. (2008). *Efficient Asset Management*. [^Michaud2008]
...
-   Becker, Y, Halffmann, P., & Sch&ouml;bel, A. (2024). Risk management in multi-objective portfolio optimization under uncertainty. [^Becker2024]
-   Fransisca, D.C., Sukono, Chaerani, D., & Halim, N.A. (2024). Robust portfolio mean-variance optimization for capital allocation in stock investment using the genetic algorithm: A systematic literature review. [^Fransisca2024]
-   Kristensen, L. & Vorobets, A. (2024). Portfolio optimization and parameter uncertainty. [^Kristensen2024]
-   Oliveira, D.C., Guzman, G., & Firoozye, N. (2025). (Non-parametric) bootstrap robust optimization for portfolios and trading strategies. [^Oliveira2025]

[^Becker2024]: @Becker_2024_Risk_management_in_multi_objective_portfolio\.
...
[^Fransisca2024]: @Fransisca_2024_Robust_portfolio_mean_variance_optimization\.
[^Kristensen2024]: @Kristensen_2024_Portfolio_optimization_and_parameter_uncertainty\.
[^Lo2002]: @Lo_2002_The_statistics_of_Sharpe_ratios\.
[^Michaud1989]: @Michaud_1989_The_Markowitz_optimization_enigma_Is_optimized\.
[^Michaud2008]: @Michaud_2008_Efficient_Asset_Management\.
[^Oliveira2025]: @Oliveira_2025_Non_parametric_bootstrap_robust_optimization\.
...
This is the subject of @sec-forecasting.
