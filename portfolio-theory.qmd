...
-   Expected return: $\hat{r}_{i} = \bar{r}_\mathrm{f} + \sum_{j} \hat{\beta}_{ij} \, \bar{F}_{j}$, where looking forward, it is usually assumed that $\alpha_{i} = 0$.

TODO: It can be shown that

$$ \hat{\beta}_{ij} = \frac{ \mathrm{Cov}(r_{i}, F_{j}) }{ \mathrm{Var}(F_{j}) } = \mathrm{Cor}(r_i, F_{j}) \: \frac{\sigma_i}{\sigma^{F}_{j}} $$
...
TODO: Actually, the above is wrong; only true for the univariate case! The general formula is

$$ \hat{\beta}^{\intercal}_{ij} = \mathrm{Cov}(F, F)^{-1}_{ik} \, \mathrm{Cov}(F, r)_{kj} $$
