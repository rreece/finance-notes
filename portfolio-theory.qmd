...
Let us look at the minimum variance portfolio, which does not depend on us estimating the returns.
...
$$ \left[ \vec{w}_\mathrm{IVP} \right]_{i} = \frac{1}{\eta} \, \frac{1}{\sigma_{i}^{2}} $$

where $\eta$ is a normalization constant, $\eta = \sum_{i} \frac{1}{\sigma_{i}^{2}}$.
...
-   Estrada, J. (2007). [Mean-semivariance optimization: A heuristic approach](https://ssrn.com/abstract=1028206). [^Estrada2007]
...
[^Estrada2007]: @Estrada_2007_Mean_semivariance_optimization_A_heuristic\.
